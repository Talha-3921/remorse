...
def _fallback_card(category: dict, out_path: Path) -> str:
    ...
    y = 190
    blocks = [("1. Search manually", dorks), ("2. Verify exact match", steps[:1]), ("3. Request removal", steps[1:] or steps)]
    for heading, lines in blocks:
        draw.rounded_rectangle((48, y, 976, y + 205), radius=18, outline="#06283D", width=5, fill="#F7FAFC")
        ...
        wrapped = textwrap.wrap(text, width=48)[:4]
...
def generate_visual_card(category: dict, style_prefix: str) -> str:
    ...
    prompt = (
        ...
        f"Use these short query hints without tiny text: {dork_preview}\n"
